...
import { cn, formatDate, PRODUCTION_STATUSES } from "@/shared/lib/utils"
...
import type { ItemRow as ItemRowType, UtilizationRow } from "./inventory-client"

function isOpportunitySigned(status: string): boolean {
  return status === "EL_FULLY_SIGNED" || (PRODUCTION_STATUSES as readonly string[]).includes(status)
}

function OpportunityTag({ status }: { readonly status: string }) {
  const signed = isOpportunitySigned(status)
  return (
    <span
      className={cn(
        "inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-medium shrink-0",
        signed ? "bg-green-100 text-green-700" : "bg-blue-100 text-blue-700"
      )}
    >
      {signed ? "Allocated" : "Reserved"}
    </span>
  )
}
...
export function ItemRow({
  item, isReadOnly, isAdmin, onUtilize, onEditUtilization, onDeleteUtilization, onDeleteItem, onOpenOpportunity,
}: Props) {
  ...
  return (
    <div className="border-b border-gray-100 last:border-b-0">
      <div className="flex items-center gap-3 py-2.5 px-1">
        ...
        <span className="text-xs text-gray-400 shrink-0">{utilizedCount} utilization{utilizedCount === 1 ? "" : "s"}</span>
        <span className="text-sm text-gray-600 tabular-nums shrink-0 whitespace-nowrap">
          {item.remainingQuantity} / {item.originalQuantity} remaining
        </span>
        {!isReadOnly && (
          <div className="flex items-center gap-1 shrink-0">
            <Button variant="primary" size="sm" onClick={() => onUtilize(item)} disabled={item.remainingQuantity <= 0}>
              Utilize
            </Button>
            {isAdmin && (
              <button
                      ...
                      <td className="px-3 py-2 align-middle">
                        {u.opportunity ? (
                          <div className="flex items-center gap-1.5 min-w-0">
                            ...
                            <OpportunityTag status={u.opportunity.status} />
                          </div>
                        ) : (
                          <span className="text-gray-300">—</span>
                        )}
